...
for _try in 1 2 3 4; do
  python3 "$EXP/score_cohort.py" >> "$LOG" 2>&1 && break
  echo "[$(date '+%F %T')] score_cohort try $_try failed (likely transient network) — retrying in $((_try*120))s" >> "$LOG"
  sleep $((_try*120))
  [ $_try -eq 4 ] && { echo "[$(date '+%F %T')] score_cohort FAILED after 4 tries" >> "$LOG"; touch "$EXP/pipeline_error"; exit 1; }
done

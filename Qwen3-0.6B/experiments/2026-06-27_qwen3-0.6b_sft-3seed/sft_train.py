...
class SFTPackedDataset(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, stream: np.ndarray, metas: list[dict], seq_len: int, no_mask: bool = False,
        ...
        for m in metas:
            ...
            pl = 0 if no_mask else min(pl, nt)   # --no_mask control: train on ALL tokens (iso-FLOP cont-pretrain)
...
def parse_args():
    ...
    ap.add_argument("--no_mask", action="store_true", help="iso-FLOP CONTROL: train on ALL tokens (no prompt mask) = continued-pretrain")
...
def main():
    ...
    ds = SFTPackedDataset(stream, metas, a.seq_len, no_mask=a.no_mask, limit_samples=lim)
    del stream

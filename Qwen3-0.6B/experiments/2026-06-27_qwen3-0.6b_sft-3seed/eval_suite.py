...
MODEL_FILE = r"/home/yashb98/Downloads/BuildFromScratch/Qwen3-0.6B/model.py"         # absolute path to the build's model file
...
TARGET_CKPT = r"/home/yashb98/Downloads/BuildFromScratch/Qwen3-0.6B/experiments/2026-06-27_qwen3-0.6b_sft-3seed/checkpoint_sft_seed0.pt"       # checkpoint under evaluation (absolute)
BASELINE_CKPT = r"/home/yashb98/Downloads/BuildFromScratch/Qwen3-0.6B/builds/2026-06-08_reproduce-faithful_qwen3-0.6b/checkpoint_qwen3_baseline2tpp.pt"   # comparison ckpt (absolute); "" -> self-floor mode.
                                       # For OBJECTIVE=finetune this IS the BASE the FT started from.
OBJECTIVE = "finetune"            # finetune | pretrain-ablation (§C13) — gates Section 4
OUT_DIR = r"/home/yashb98/Downloads/BuildFromScratch/Qwen3-0.6B/experiments/2026-06-27_qwen3-0.6b_sft-3seed/eval/sft_seed0"               # <MODEL_DIR>/experiments/<RUN_ID>/eval
...
def load_model_module():
    ...
    # Register BEFORE exec: a model.py using @dataclass (e.g. Qwen3Config) makes
    # the dataclass machinery look up sys.modules[cls.__module__].__dict__ during
    # class creation; if the module isn't registered yet that is None -> crash.
...
def main():
    ...
    results = {
        ...
        "forgetting": {}, "not_run": {},
        # top-level headline scalars filled from the first corpus (deploy gate)
        "bpb": None, "headline_ppl": None, "noise_floor_abs": None,
        "headline_corpus": None,
    }
    ...
    for section, corpus_id, text in corpora:
        try:
            ...
            t_ppl, n_tok, t_sum_nll, t_bytes = ppl_ids(target, ids, tok=tokenizer)
            t_bpb = (t_sum_nll / math.log(2)) / t_bytes if t_bytes else None
            fl = noise_floor(floor_model, ids)
            entry = {"corpus_id": corpus_id, "target": t_ppl, "n_tokens": n_tok,
                     "bpb": t_bpb, "n_bytes": t_bytes}
            ...
            # top-level headline scalars (the §C23.4 deploy gate reads these via
            # eval_gate._lift_metrics). The FIRST corpus (wikitext2_val) is the
            # cross-tokenizer headline; BPB is the comparable number.
            if "bpb" not in results or results.get("headline_corpus") is None:
                results["bpb"] = t_bpb
                results["headline_ppl"] = t_ppl
                results["noise_floor_abs"] = fl["floor_abs"]
                results["headline_corpus"] = corpus_id
            save()
...
if __name__ == "__main__":
    ...
    ap.add_argument("--target", default=None, help="override TARGET_CKPT (score another arm, same base+suite)")
    ap.add_argument("--out", default=None, help="override OUT_DIR (per-target results dir)")
    _a = ap.parse_args()
    if _a.target:
        TARGET_CKPT = _a.target
    if _a.out:
        OUT_DIR = _a.out
    if _a.smoke:
        smoke()

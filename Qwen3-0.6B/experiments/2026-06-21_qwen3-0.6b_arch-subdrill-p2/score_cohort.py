...
ARMS = ("baseline", "vr", "ln", "hg")
# Phase-2 sub-drill: each arm = exactly one IMU-1 arch flag on (baseline = all off).
# ln adds +0 params (parameter-free depth-scaling) but MUST load with the flag on
# so the forward matches training; vr/hg add params -> strict load needs the flag.
ARM_FLAGS = {"baseline": (False, False, False), "vr": (True, False, False),
             "ln": (False, True, False), "hg": (False, False, True)}
...
def main():
    ...
    for cell in CELLS:
        vr, ln, hg = ARM_FLAGS[cell.rsplit("_seed", 1)[0]]

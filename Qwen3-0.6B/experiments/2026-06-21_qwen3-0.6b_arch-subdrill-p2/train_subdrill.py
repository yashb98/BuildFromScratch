...
def main():
    ...
    # Phase-2 arch sub-component drill: each IMU-1 arch tweak as an INDEPENDENT flag
    ap.add_argument("--value_residual", type=int, default=0)      # 1 = on
    ap.add_argument("--layernorm_scaling", type=int, default=0)   # 1 = on
    ap.add_argument("--head_gating", type=int, default=0)         # 1 = on
    ...
    # --arch imu1 still turns ALL three on (Phase-1 bundle); the sub-drill sets
    # each flag independently. baseline = all off = faithful (bit-exact verified).
    bundle = (args.arch == "imu1")
    vr = bool(args.value_residual) or bundle
    ln = bool(args.layernorm_scaling) or bundle
    hg = bool(args.head_gating) or bundle
    ...
    if args.dry_run:
        ...
        cfg = M.Qwen3Config(num_hidden_layers=2, hidden_size=128, intermediate_size=256,
                            ...
                            use_value_residual=vr, use_layernorm_scaling=ln,
                            use_head_gating=hg)
        seq, steps = 16, 5
        ...
        cfg = M.Qwen3Config(use_value_residual=vr, use_layernorm_scaling=ln,
                            use_head_gating=hg)

...
def cooldown_factor(step, total, warmup, floor):
    """Mid-training ANNEAL cooldown (plan.md §3): short warmup to the START lr, then a
    1-sqrt decay from 1.0 down to `floor` (=end_lr/peak_lr, a small NON-zero floor ~10%)
    over the whole window. lr(step)=peak_lr*factor; with peak_lr=2.5e-4, floor=0.1 ->
    2.5e-4 -> 2.5e-5. NOT decay-to-zero (2511.18903: premium data stranded under a
    collapsed LR is wasted) and NO re-warm above the start lr (avoids the re-warm confound)."""
    if step < warmup:
        return step / max(1, warmup)
    prog = (step - warmup) / max(1, total - warmup)      # 0..1
    return floor + (1.0 - floor) * (1.0 - prog ** 0.5)   # 1-sqrt: 1.0 at prog0 -> floor at prog1
...
def main():
    ...
    ap.add_argument("--schedule", choices=["cosine", "wsd", "cooldown"], default="cosine")
    ...
    ap.add_argument("--data", choices=["fineweb","dclm","mix"], default="fineweb")  # data-selection A/B
    ...
    ap.add_argument("--init_from", default=None, help="load BASE weights only (fresh optimizer + schedule @ step 1) — the anneal start")
    ...
    elif not args.dry_run and args.init_from and pathlib.Path(args.init_from).exists():
        bck = torch.load(args.init_from, map_location=device, weights_only=False)
        model.load_state_dict(_strip(bck["model"]), strict=True)   # base weights only, fresh optimizer + cooldown @ step 1
        log(f"INIT-FROM base weights {args.init_from} (strict load OK; fresh optimizer; cooldown @ step 1)", log_path)
    ...
    if args.dry_run:
        ...
        if args.data in ("dclm", "mix"):
            import numpy as np, glob as _g
            DD = str(ROOT / "research/datasets/data-selection-dclm-edu")
            sh = sorted(_g.glob(DD + "/shard_*.bin"))
            dclm = np.concatenate([np.fromfile(p, dtype=np.uint32) for p in sh]).astype(np.int64)
            if args.data == "mix":
                # 50/50 by tokens: half the budget from dclm, half streamed from FineWeb-Edu
                ...
                train_tokens = torch.from_numpy(dclm)
                val_tokens = torch.from_numpy(np.fromfile(DD + "/eval_tokens.bin", dtype=np.uint32).astype(np.int64)[:300_000])
                log(f"DATA=dclm-edu: {len(dclm):,} train + {len(val_tokens):,} val tokens (shards {len(sh)})", log_path)
        else:
    ...
    for step in range(start_step, steps + 1):
        ...
        if args.schedule == "cosine":
            factor = cosine_factor(step, steps, args.warmup_steps, floor)
        elif args.schedule == "cooldown":
            factor = cooldown_factor(step, steps, args.warmup_steps, floor)
        else:

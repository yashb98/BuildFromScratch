#!/usr/bin/env python3
"""Score the 6 anneal checkpoints (2 arms × 3 seeds) + the no-anneal BASE reference
through the text-lm-v2 suite logic (same SEQ/STRIDE/corpora/bits_per_byte as
eval-harness) → per-cell BPB+PPL.

Arms (research/midtraining/plan.md §2): mix = premium 50/50 cooldown (treatment),
fineweb = iso-token FineWeb-Edu-only cooldown (control). The `base` cell is the
un-annealed checkpoint_qwen3_baseline2tpp.pt scored under the identical windows —
the §C25 short-context non-regression reference (an eval reuse, not a re-train).

Run AFTER cohort.done (all 6 arm_<cell>.done present). Sequential on the single GB10
...
ARMS = ("fineweb", "mix")   # anneal cohort: iso-token control / premium 50-50 treatment
CELLS = [f"{arm}_seed{s}" for arm in ARMS for s in (0, 1, 2)]
BASE_CKPT = (pathlib.Path(__file__).resolve().parents[2] / "builds" /
             "2026-06-08_reproduce-faithful_qwen3-0.6b" / "checkpoint_qwen3_baseline2tpp.pt")
...
def main():
    ...
    if not BASE_CKPT.exists():
        print(f"NOT READY — base reference missing: {BASE_CKPT}"); return 1
    ...
    for cell in CELLS + ["base"]:
        cfg = M.Qwen3Config(use_value_residual=False, use_layernorm_scaling=False,
                            use_head_gating=False)   # both arms faithful arch; only the DATA differs
        t0 = time.time()
        ckpt_path = BASE_CKPT if cell == "base" else HERE / f"checkpoint_{cell}.pt"

...
| exploratory partial-RoPE 0.10 | 50.71 | in progress (latest eval) |

**Ranking (lower = better):** modernized (NorMuon bundle) < faithful < partial-RoPE 0.25 < partial-RoPE 0.10.

Partial RoPE (both 0.25 and 0.10) does **not** beat the faithful baseline at this scale; the NorMuon-bundle modernized build is the clear winner. Official cross-run eval-harness numbers run once the GPU frees.

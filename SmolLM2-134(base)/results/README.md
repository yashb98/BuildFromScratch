...
`../verify.py` / `../model.py`). No values are typed in by hand.

## Headline numbers (also in `summary.json`)

| Metric | Value |
|---|---|
| Unique parameter count | **134,515,008** (target match ✓) |
| `lm_head` tied to `embed_tokens` | True |
| `max │Δlogits│` vs HuggingFace `LlamaForCausalLM` | **0.000e+00** (fp32, same input) |
...
| Demo from-scratch loss (start → end, 150 steps) | 11.254 → **6.321** |
...
- `perplexity.json` — sliding-window CE perplexity on wikitext-2-raw-v1
  validation, 62,403 target tokens, seq=1024 stride=512. Ours vs HF agree to
  6 decimal places.
